...
async def create_document(library_id: int, document_create: DocumentCreate):
    ...
    logger.info(f"Creating document: name='{document_create.name}'")
    try:
        result = await StorageService.documents().create(library_id, document_create)
        logger.info(f"Document created: name='{document_create.name}' id={result.id}")
        return result
...
async def delete_document(library_id: int, document_id: int):
    ...
    chunks = await StorageService.chunks().list_by_document(document.id)
    chunk_ids = [c.id for c in chunks]

    # Delete chunks from storage
    await StorageService.chunks().delete_by_document(document.id)

    # Remove from vector index
    if chunk_ids:
        await SearchService.delete_vectors(library_id, chunk_ids)

    await StorageService.documents().delete(document.id)
